# coding=utf-8
# This is the battle_controller.py used for WoWs 15.2 and 15.3.
# Copy this file to:
#   replay_unpack/clients/wows/versions/15_2_0/battle_controller.py
#   replay_unpack/clients/wows/versions/15_3_0/battle_controller.py
# (and future versions, adjusting as needed)
#
# Changes from the original 14_11_0 battle_controller.py:
#
# 1. Removed debug exit() call in _set_health (was: if entity.id == 959830: exit())
#
# 2. Updated _on_consumable_used signature:
#    - Old: (self, entity, consumableType, workTimeLeft)
#    - New: (self, entity, consumableUsageParams, workTimeLeft)
#    The consumable slot ID is now in byte 1 of consumableUsageParams.
#
# 3. Fixed _update_ribbons to store ribbons under shipId instead of avatar.id
#    so the renderer can find them by owner_vehicle_id.
...
class BattleController(IBattleController):
    ...
    def _on_consumable_used(self, entity: Entity, consumableUsageParams, workTimeLeft):
        """
        Called when a ship activates a consumable.

        In WoWs 15.2+, consumableUsageParams is a 2-byte blob where:
        - byte 0: always 0x01 (activation flag)
        - byte 1: consumable slot ID (matches keys in abilities.json id_to_index)
        """
        consumables = self._acc_consumables.setdefault(entity.id, [])
        consumable_id = (
            consumableUsageParams[1]
            if isinstance(consumableUsageParams, (bytes, bytearray)) and len(consumableUsageParams) >= 2
            else 0
        )
        consumables.append(
    ...
    def _set_health(self, entity: Entity, health):
        # NOTE: Original 14_11_0 had a debug exit() call here that was removed.
        self._dict_vehicle[entity.id] = self._dict_vehicle[entity.id]._replace(
            health=health
        )
    ...
    def _update_ribbons(self, avatar, ribbons_state):
        """
        Store ribbons under the ship's vehicle ID (shipId) rather than
        the avatar entity ID, so the renderer can look them up correctly
        via owner_vehicle_id.
        """
